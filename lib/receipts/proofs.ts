...
// ─── Machine line: single source for the generated sentence + its recogniser ──
//
// The pack notice's machine line (`{monthLabel} の領収証憑一式をお送りします。`)
// is generated by buildPackNotice AND recognised by the preflight layout
// discriminator (pack-preflight.ts noticeMessageLayout). These MUST stay in
// sync — a copy edit to the wording (e.g. お送りします → お送りいたします) that
// updates the builder but not the recogniser would silently reclassify every
// new-layout notice as old and block delivery. One constant feeds both: the
// builder constructs from it, the predicate matches against it. NOTICE-specific
// — the email's machine line differs (…を添付にて…) and is not parsed by
// preflight; do not merge the two.
const MACHINE_LINE_SUFFIX = " の領収証憑一式をお送りします。";

export function packNoticeMachineLine(monthLabel: string): string {
  return monthLabel + MACHINE_LINE_SUFFIX;
}

export function isPackNoticeMachineLine(line: string): boolean {
  return line.endsWith(MACHINE_LINE_SUFFIX);
}
...
export function buildPackNotice(input: PackNoticeInput, names: PackNames): string {
  ...
  lines.push("【今月のご連絡】");
  lines.push(packNoticeMachineLine(input.monthLabel));
  lines.push("");
