...
- [x] Create `middleware.ts` using `clerkMiddleware()`. The matcher explicitly includes
      ...
      matcher (separate bearer-token scheme). *(Phase 3 correction: one exception —
      `/api/mobile/auth/complete-pairing` was added to the matcher because its handler calls
      `requireReceiptsActor()` → `auth()`, which requires `clerkMiddleware` to have run for the request;
...
- [x] **`complete-pairing` operator approval runs through Clerk middleware (correction).** The earlier claim here — that "`auth()` reads the Clerk session cookie directly, so `/api/mobile/*` can stay outside the matcher and the operator-approval step is still Clerk-gated" — was **false**. Clerk's server-side `auth()` REQUIRES `clerkMiddleware()` to run for the request; it does not read the session cookie on its own. `@clerk/nextjs` `dist/types/app-router/server/auth.d.ts` documents `auth()` as "Requires `clerkMiddleware()` to be configured", and the runtime raises `auth_signature_invalid` ("Clerk: Unable to verify request, this usually means the Clerk middleware did not run. Ensure Clerk's middleware is properly integrated and matches the current route.") when it didn't. Because `middleware.ts` matched no `/api/mobile/*` route, `app/api/mobile/auth/complete-pairing/route.ts`'s `requireReceiptsActor(request.headers)` → `auth()`/`clerkClient()` returned no usable identity, so a signed-in operator's "Pair this iPhone" POST could fail. **Fix:** `/api/mobile/auth/complete-pairing` is now the one `/api/mobile/*` entry in `config.matcher`; it is matched and NOT public, so `clerkMiddleware` runs and `auth.protect()` gates it. Real chain: `browser POST /api/mobile/auth/complete-pairing` → exact matcher entry → `auth.protect()` → route handler → `requireReceiptsActor()` → `auth()`/`clerkClient()`. Every other `/api/mobile/*` endpoint stays outside the matcher (device-bearer / pairing-code auth). The routing config is exported as `config.matcher` and `PUBLIC_ROUTES` and asserted by `tests/middleware-routing.test.ts`. A real-device positive pairing test remains deferred to post-deploy verification — NOT claimed as passed.
